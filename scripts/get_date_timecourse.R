...
mobile <- WDI(indicator='gwp1_n') %>% #"Access to a mobile phone (% age 15+)" 
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = gwp1_n) %>% 
...
internet <- WDI(indicator = "IT.NET.USER.ZS") %>% #"Individuals using the Internet (% of population)"
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = `IT.NET.USER.ZS`) %>% select(country, year, value) %>%
...
internet2 <- WDI(indicator = "gwp2_n") %>% #"Access to internet (% age 15+)"  
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = "gwp2_n")  %>% select(country, year, value) %>%
...
internet3 <- WDI(indicator = "IT.NET.BNDW") %>% #"International Internet bandwidth (Mbps)" 
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = "IT.NET.BNDW") %>% select(country, year, value) %>%
...
comp <- WDI(indicator = "IT.PC.HOUS.ZS") %>% # "Homes with a personal computer (%)" 
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = "IT.PC.HOUS.ZS") %>% select(country, year, value) %>%
  mutate(vars = "comp")
...
lit <- WDI(indicator = "LO.PIAAC.LIT") %>% # PIAAC: Mean Adult Literacy Proficiency. Total
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = "LO.PIAAC.LIT") %>% select(country, year, value) %>%
  mutate(vars = "lit") 
...
lit2 <- WDI(indicator = "SE.ADT.LITR.ZS") %>% # "Literacy rate, adult total (% of people ages 15 and above)"
  ...
  filter(year %in% max(year):(max(year)-5)) %>%
  filter(year > 2000) %>%
  rename(value = "SE.ADT.LITR.ZS") %>% select(country, year, value) %>%
  mutate(vars = "lit2")  
...
write_csv(data, "data/wbdata_timecourse.csv")
